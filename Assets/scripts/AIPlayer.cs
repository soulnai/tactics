...
public class AIPlayer : Player {
	...
	public override void TurnUpdate ()
	{
		if (positionQueue.Count > 0) {
			...
			else if (currentUnitAction != unitActions.moving && movementToAttackTilesInRange.Where(x => GameManager.instance.players.Where (y => y.GetType() != typeof(AIPlayer) && y.HP > 0 && y != this && y.gridPosition == x.gridPosition).Count() > 0).Count () > 0) {
			...
			else if (currentUnitAction != unitActions.moving && movementTilesInRange.Where(x => GameManager.instance.players.Where (y => y.GetType() != typeof(AIPlayer) && y.HP > 0 && y != this && y.gridPosition == x.gridPosition).Count() > 0).Count () > 0) {
